...
| `Directory not empty; pass --force`                                               | The selected target folder has prior files.                                             | If the current folder is non-empty, infer an available app subfolder and retry there. Use `--force` only when the user explicitly asks for it. |
